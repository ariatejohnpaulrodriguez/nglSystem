...
// Check if transfer_id is set in the GET request and is a number
if (isset($_GET['transfer_id']) && is_numeric($_GET['transfer_id'])) {
    $transferID = $_GET['transfer_id'];

    // SQL query to retrieve transfer_id, from_company image, from_company name, from_company address, and attention
    $query = "SELECT
          ...
          LEFT JOIN dates AS posting_dates ON transfers.posting_date = posting_dates.date_id
          LEFT JOIN dates AS delivery_dates ON transfers.delivery_date = delivery_dates.date_id
    ...
    if ($result->num_rows > 0) {
        ...
        $imagePaths = '../../' . $row['from_company_image']; // Adjust the path to the image directory
        if (!empty($row['from_company_image']) && file_exists($imagePaths)) {
            // Get original image dimensions
            list($originalWidth, $originalHeight) = getimagesize($imagePaths);
            $maxHeight = 30;
            $maxWidth = 50;
            ...
            $pdf->Image($imagePaths, 15, 10, $width, $height); // Adjust the coordinates and size
        } else {
        ...
        $pdf->Output('transfer.pdf', 'I'); // Output the PDF
    } else {
        echo "No data found for transfer ID: " . htmlspecialchars($transferID);
    }
} else {
